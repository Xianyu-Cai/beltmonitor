...
export class CameraService {
  ...
  getHlsUrl(cameraID: number): string {
    return `http://localhost:${2000 + cameraID}/hls_output/camera_${cameraID}/output.m3u8`;
  }

  // async updateCameraStream(cameraID: number, streamUrl: string) {
  //   await this.cameraRepo.update({ id: cameraID }, { streamUrl });
  // }
}
